...
def seg(hsid,points,labels):    
    checkpoint = "./checkpoints/sam2.1_hiera_large.pt"
    model_cfg = "configs/sam2.1/sam2.1_hiera_l.yaml"
    predictor = build_sam2_video_predictor(model_cfg, checkpoint)

    video_path = f"exp_data/{hsid}/face_crop.mp4"
    out_mask_dir = f"exp_data/{hsid}/sam2_mask"
    ...
    os.makedirs(out_mask_dir,exist_ok=True)
    merget_video(f"{out_mask_dir}/%6d.jpg", f"exp_data/{hsid}/sam2.mp4")
...
if __name__ == '__main__':
    hsid = "HSID48"
    # for hsid,points,labels in dataset[1:]:
    #     seg(hsid,points,labels)
